...
class AgentNotAvailableError(EvalError):
    ...
    def __init__(self, agent_name: str = "agent", detail: str = "") -> None:
        super().__init__(
            ...
            what=f"{agent_name} CLI not found on PATH",
            why=detail or "The agent CLI must be installed and on PATH to run evaluations",
            fix=f"Install {agent_name} and ensure it is on your PATH",
        )
    ...
    def check_available(self) -> None:
        ...
        if shutil.which(self.CLI_NAME) is None:
            raise AgentNotAvailableError(
                self.CLI_NAME,
                "Install from https://docs.anthropic.com/en/docs/claude-code or add it to your PATH",
            )
...
def register_runner(name: str, runner_class: type[AgentRunner]) -> None:
    ...
    if not (isinstance(runner_class, type) and issubclass(runner_class, AgentRunner)):
        raise EvalError(
            code="E_BAD_RUNNER",
            what=f"runner_class must be a subclass of AgentRunner, got {runner_class}",
            why="Only AgentRunner subclasses can be registered",
            fix="Pass a class that inherits from AgentRunner",
        )
...
def get_runner(name: str = "claude") -> AgentRunner:
    ...
    if name not in _RUNNER_REGISTRY:
        ...
        raise EvalError(
            ...
            what=f"No agent runner registered as {name!r}",
            why=f"Available runners: {available}",
            fix=f"Use one of: {available}, or register a custom runner with register_runner()",
        )
