...
def run_functional_eval(
    ...
    try:
        ...
        raise EvalError(
            code="E_LOAD_EVALS",
            what=f"Failed to load evals from {evals_file}",
            why=str(e),
            fix="Ensure the evals.json file exists and contains valid JSON",
        )

    if not eval_cases:
        raise EvalError(
            code="E_NO_EVALS",
            what="No eval cases found",
            why=f"The evals file at {evals_file} is empty or contains no valid cases",
            fix="Add eval cases to evals.json, or run 'skillctl init' to generate a scaffold",
        )
